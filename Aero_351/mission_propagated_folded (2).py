...
def phasing_sequence_propagated(r_start_on_target, k2, a_tgt, N, a_drift):
    ...
    return {"segments": tracks, "end_state": (rG, v_recirc), "dv": dv_terms}

# -------------------------
# Leg 3: fully 3D Lambert (no explicit plane change)
# -------------------------
def leg3_3d_lambert_propagated(t0, A, B):
    # A,B are target dicts (leoA -> leoB)
    r1 = A["a"]; r2 = B["a"]
    # Pick a position on A's circle at the planes' intersection for determinism
    rhat, kA, kB = node_line(A["i"], A["Omega"], B["i"], B["Omega"])
    r_dep, v_dep = circ_state_on_line(r1, kA, rhat)  # pre-burn circular in A

    # Choose TOF scale and scan
    ...
    for s in np.linspace(0.4, 1.6, 49):
        ...
        try:
            ...
            continue
        dv_dep = np.linalg.norm(v1_tr - v_dep)      # includes plane change component
        dv_arr = np.linalg.norm(v_tgt_arr - v2_tr)  # includes plane change component
        dv_tot = dv_dep + dv_arr
        if (best is None) or (dv_tot < best[0]):
            best = (dv_tot, tof, t_arr, v1_tr, v2_tr, dv_dep, dv_arr, r_tgt_arr, v_tgt_arr)

    if best is None:
        raise RuntimeError("Lambert scan failed for 3D leg.")

    dv_total, tof_best, t_arrival, v1_tr, v2_tr, dv_dep, dv_arr, r_tgt_arr, v_tgt_arr = best
...
if __name__ == "__main__":
    ...
    # ---- Leg 3: 33393U -> 52939U (3D Lambert; no explicit plane-change) ----
    t3_dep = t0 + timedelta(seconds=leg1["tof"] + dwell1 + leg2["tof"] + dwell2)
    leg3 = leg3_3d_lambert_propagated(t3_dep, leoA, leoB)
